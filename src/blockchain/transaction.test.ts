...
describe('Transaction', () => {
  ...
  describe('Transaction Builder',  () => {
    it('should be able to create a transaction successfully', async () => {
      ...
      await blockchainClient
        .transaction()
        .addOperation('create_account', user2.publicKey)
        .sign(user2)
        .send();

      await blockchainClient
        .transaction()
        .addOperation('transfer_balance', user1.publicKey, user2.publicKey, 10)
        .sign(user1)
        .send();

      const user2Balance = await blockchainClient
        .query()
        .name('get_balance')
        .addParameter('pubkey', user2.publicKey.toString('hex'))
        .send();
